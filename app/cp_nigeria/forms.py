...
class DemandOptionsForm(forms.Form):
    ...
    help_text_exclude_hhs = "The simulation will only consider the demand generated by public facilities and productive users. A system sized to satisfy this demand will be smaller and can be a good first step in electrifying your community."
    ...
    exclude_households = forms.BooleanField(
        widget=forms.CheckboxInput(attrs={"data-bs-toggle": "tooltip"}),
        required=False,
        label="Exclude households from mini-grid simulation" + question_icon.format(help_text_exclude_hhs),
    )

    demand_coverage_factor = forms.FloatField(
        required=False,
        label="Minimal percentage of the household demand to fulfill" + question_icon.format(help_text_demand),
    )

    def __init__(self, *args, **kwargs):
        single_hh_demand = kwargs.pop("single_hh_demand", False)
        ...
        # If no community is selected, only a checkbox to exclude/include household demand will be shown
        # If a community is selected, a drop-down to select the specific threshold for SHS will be shown
        if single_hh_demand:
            self.fields.pop("shs_threshold")
        else:
            self.fields.pop("exclude_households")
