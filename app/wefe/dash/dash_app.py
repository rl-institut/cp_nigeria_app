...
# from oemof_tabular_plugins.general.post_processing.post_processing import (
#     extract_table_from_results,
...
#
# def prepare_tables():
#     extract_table_from_results(calculator.df_results, RESULT_TABLE_COLUMNS["costs"])
#
#     capacities_table = extract_table_from_results(results_by_flow, RESULT_TABLE_COLUMNS["capacities"])
#
#
...
#                 table.drop(columns=["carrier", "direction", "facade_type"], inplace=True)
...
#                     opts["stackgroup"] = "negative_sign" if negative_sign < 0 else "positive_sign"
#
#                     fig.add_trace(
#                         go.Scatter(
#                             x=g.index,
...
#                 if capacity_battery != 0:
#                     soc_battery = (
#                         solph.views.node(results, node=bus)["sequences"][(("battery", "None"), "storage_content")]
#                         / capacity_battery
#                     )
#                 else:
#                     soc_battery = solph.views.node(results, node=bus)["sequences"][
...
#
#                 fig.add_trace(go.Scatter(x=soc_battery.index, y=soc_battery.values, name="soc battery"))
#                 fig.add_trace(
#                     go.Scatter(
#                         x=soc_battery.index,
...
#     @demo_app.callback(
#         # The value of these components of the layout will be changed by this callback
#         [Output(component_id=f"{s}-service-div", component_property="style") for s in services],
#         # Triggers the callback when the value of one of these components of the layout is changed
#         Input(component_id="service_select", component_property="value"),
#     )
#     def change_visibility_value(val):
#         answer = [
#             (service__item_style | {"display": "block"} if s in val else service__item_style | {"display": "none"})
#             for s in services
#         ]
#         return answer
#
